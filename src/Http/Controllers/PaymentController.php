...
class PaymentController
{
    ...
    public function requery(Request $request)
    {
        ...
        if ($response->failed()) {
            ...
            $resultHeader = $response->header('content-type');

            if ($resultHeader == 'application/json' && $statusCode == 422) {
                $errors = collect($response->json()['errors'])->mapWithKeys(function ($value, $key) {
                    return ['bc_console.' . $key => $value];
                })->all();
        ...
        return $resultBody['output']['transactionsList']['recordsListData']??[0];
    }
}